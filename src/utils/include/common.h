...
//transfor cv rotation matrix 3x3 to Mat3x3, so it cannot be used to trans rotation vector to Mat3x3
...
// This function needs rvec to be 3x1 cv::Mat (rotation vector)
...
// Output rvec is a rotation vector (3x1 cv::Mat)
inline void SE3_to_rvec_tvec(const SE3 pose, cv::Mat &rvec, cv::Mat &tvec)
{
    Quaterniond q = pose.unit_quaternion();
    cv::Mat t_,r_;
    tvec = Vec3_to_cvMat(pose.translation()); // SE3-> cv::Mat translation
    cv::Mat R_ = Mat3x3_to_cvMat(q.toRotationMatrix());//SE3 -> cv::Mat
    cv::Rodrigues ( R_, rvec ); //cv::Mat rotation -> cv::Mat rotation vector
}
