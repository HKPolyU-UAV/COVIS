...
*/
RVIZFrame::RVIZFrame(ros::NodeHandle& nh,  string poseTopicName, string ptsTopicName, size_t &AgentId_, string &AgentFrameId,
                     int bufferPose,
                     int bufferPts)
{
    ...
    *ss1 << "/Agent" << AgentId_ << poseTopicName;
    *ss2 << "/Agent" << AgentId_ << ptsTopicName;

    pose_pub   = nh.advertise<geometry_msgs::PoseStamped>(ss1->str(), bufferPose);
    marker_pub = nh.advertise<visualization_msgs::Marker>(ss2->str(), bufferPts);
    this->frame_id_pose = AgentFrameId;
    this->frame_id_pts = AgentFrameId;
    delete ss1;
    delete ss2;
