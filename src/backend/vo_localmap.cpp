...
private:
    ...
    void frame_callback(const covis::KeyFrameConstPtr& msg)
    {
        ...
        if(optimizer_state==OPTIMIZING)
        {
            ...
            {
                //cout << "output_sparse_map: " << std::boolalpha << output_sparse_map << endl;
                for (auto lm:lms)
                {
            ...
            if(output_sparse_map)
            {
                ...
                output.header.frame_id=AgentFrameId;
    ...
    virtual void onInit()
    {
        ...
        this->AgentId_ = static_cast<size_t>(AgentId);
        cout << "AgentId: " << AgentId << endl;
        nh.getParam("AgentFrameId", this->AgentFrameId);
        cout << "AgentFrameId: " << this->AgentFrameId << endl;
        ...
        if(output_sparse_map)
        {
            ...
            stringstream* ss;
            ss = new stringstream;
            *ss << "/Agent" << AgentId_ << "/map_cloud";
            map->header.frame_id = AgentFrameId;
            map->height = map->width = 1;
            map_pub = nh.advertise<sensor_msgs::PointCloud2>(ss->str(), 1);
            delete ss;
        }
        stringstream* ss;
        ss = new stringstream;
        *ss << "/Agent" << AgentId_ << "/vo_kf";
        sub_kf = nh.subscribe<covis::KeyFrame>(
                    ss->str(),
                    10,
...
PLUGINLIB_EXPORT_CLASS(covis_ns::LocalMapNodeletClass, nodelet::Nodelet)
