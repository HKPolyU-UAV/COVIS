...
static void writetoTUMfile(ros::Time stamp, double x, double y, double z,
                           double qw, double qx, double qy, double qz)
{
  if(enable_output_file)
  {
...
// NavPath msg save as TUM or Kitti format
void mySigintHandler(int sig)
{
    ...
    if(sub_type=="NavPath" && enable_output_file == true)
    {
        ...
        {
            cout << "record_path.poses.size(): " << record_path.poses.size() << endl;
...
//                                 pose_msg.pose.orientation.w, pose_msg.pose.orientation.x, pose_msg.pose.orientation.y, pose_msg.pose.orientation.z);
                writetoTUMfile(pose_msg.header.stamp,
                               pose_msg.pose.position.x, pose_msg.pose.position.y, pose_msg.pose.position.z,
                               pose_msg.pose.orientation.w, pose_msg.pose.orientation.x, pose_msg.pose.orientation.y, pose_msg.pose.orientation.z);
            }
            fd.close();
...
void NavPath_callback(const nav_msgs::PathConstPtr& msg)
{
        ...
        //cout << "poses: " << record_path.poses.size() << endl;
        count ++;
}
